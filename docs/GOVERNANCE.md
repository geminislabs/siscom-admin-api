...
pytest-cov enforces a **minimum 65% line coverage** on `app/` (configured in `pyproject.toml`). Current baseline is ~67%; the floor prevents regressions without requiring a large test expansion in this PR.

Excluded from measurement: Alembic migrations under `app/db/migrations/`.

Raise the floor only after adding tests in follow-up PRs (`chore/soft-foundations`, `fix/*`).
...
3. Major version PRs (FastAPI, SQLAlchemy, etc.): run `make validate` locally before merge.
